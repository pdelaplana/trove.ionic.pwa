      ...
      <CenterContainer>
        ...
        <IonList className='ion-outline'>
          <IonItem
            ...
            button
            detail
            routerLink={`/settings/loyalty/${loyaltyProgram?.id}/edit`}
          >
            <IonLabel>
              <h1>{loyaltyProgram?.name}</h1>
              <p>
                {`Earn ${loyaltyProgram?.pointsPerSpend} ${t(
                  `types.loyaltyProgramType.${loyaltyProgram?.type}`,
                  {
          ...
          <IonList className='ion-outline'>
            {loyaltyProgram?.tiers.map((tier) => (
              <IonItem
                ...
                detail
                lines='none'
                routerLink={`/settings/loyalty/${loyaltyProgram?.id}/tiers/${tier.id}`}
              >
                <IonLabel>
                  <h2>{tier.name}</h2>
                  <p>
            ...
            <IonItem
              ...
              detail
              lines='none'
              routerLink={`/settings/loyalty/${loyaltyProgram?.id}/tiers/new`}
              className='no-border'
            >
              <IonIcon
        ...
        <ContentSection title='Milestones'>
          <IonList className='ion-outline'>
            {loyaltyProgram?.milestones
              ...
              .map((milestone) => (
                <IonItem
                  ...
                  detail
                  lines='none'
                  routerLink={`/settings/loyalty/${loyaltyProgram?.id}/milestones/${milestone.id}`}
                >
                  {milestone.tierId ? (
            ...
            <IonItem
              ...
              detail
              lines='none'
              routerLink={`/settings/loyalty/${loyaltyProgram?.id}/milestones/new`}
              className='no-border'
            >
              <IonIcon
                icon={addCircleOutline}
